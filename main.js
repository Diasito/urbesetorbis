...
const els = {
  ...
  hanson: getEl("hanson"),
  orbis: getEl("orbis"),
  trismegistos: getEl("trismegistos"),
  coords: getEl("coords"),
  coords2: getEl("coords2"),
  flag: getEl("flag"),
  googlelink: getEl("googlelink"),
  OSMlink: getEl("OSMlink"),
  wikimapialink: getEl("wikimapialink"),
  darelink: getEl("darelink"),
  pecslink: getEl("pecslink"),
  pleiadeslink: getEl("pleiadeslink"),
  tpplacelink: getEl("tpplacelink"),
  topostextlink: getEl("topostextlink"),
  vicilink: getEl("vicilink"),
  wikidatalink: getEl("wikidatalink"),
  wikipedialink: getEl("wikipedialink"),
  id130: getEl("id130-value"),
  permLink: getEl("perm-link-display"),
  hello: getEl("pusto"),
  node: getEl("hopa"),
  minimapDiv: getEl("minimap"),
...
const roads = new VectorLayer({
  source: new VectorSource({
    format: new GeoJSON(),
    url: "/data/cultural/roads10.geojson",
  }),
...
const pomerium = new VectorLayer({
  ...
  style: new Style({ fill: new Fill({ color: "rgba(255,0,0,0.3)" }) }),
  minZoom: 6.9999,
  maxZoom: 10,
  opacity: 0.8,
});
...
const provimena = new VectorLayer({
  ...
  maxZoom: 8,
  opacity: 1,
  style: makeDynamicLabel("title", "", [87, 0, 127, 0.6], [255, 255, 255, 0.3]),
});
...
const createCityLayer = (sourceUrl, styleFn, minZ, maxZ) =>
  new VectorLayer({
    source: new VectorSource({ format: new GeoJSON(), url: sourceUrl }),
    style: styleFn,
    declutter: true,
    minZoom: minZ,
    maxZoom: maxZ,
...
const RomaL = createCityLayer(
  "/data/cities/1roma.geojson",
  createCityStyleFn(baseStyles.Roma, (t) => t.toUpperCase()),
  3.9999,
  10,
);
const CordubaL = createCityLayer(
  "/data/cities/2corduba.geojson",
  createCityStyleFn(baseStyles.Corduba, (t) => t),
  3.9999,
  10,
);
const ByzantiumL = createCityLayer(
  "/data/cities/3byzantium.geojson",
  createCityStyleFn(baseStyles.Byzantium, (t) => t),
  3.9999,
  10,
);
const LondiniumL = createCityLayer(
  "/data/cities/4londinium.geojson",
  createCityStyleFn(baseStyles.Londinium, (t) => t.replace(/ /g, "\n")),
  3.9999,
  10,
);
const TheodosiaL = createCityLayer(
  "/data/cities/5theodosia.geojson",
  createCityStyleFn(baseStyles.Theodosia, (t) => t.replace(/ /g, "\n")),
  3.9999,
  10,
);
const AntinoopolisL = createCityLayer(
  "/data/cities/6antinoopolis.geojson",
  createCityStyleFn(baseStyles.Antinoopolis, (t) => t.replace(/ /g, "\n")),
  4.9999,
  10,
);
const DelosL = createCityLayer(
  "/data/cities/7delos.geojson",
  createCityStyleFn(baseStyles.Delos, (t) => t.replace(/ /g, "\n")),
  5.9999,
  10,
);
const PityousL = createCityLayer(
  "/data/cities/8pityous.geojson",
  createCityStyleFn(baseStyles.Pityous, (t) => t.replace(/ /g, "\n")),
  6.9999,
  10,
);
const ShemakhaL = createCityLayer(
  "/data/cities/9shemakha.geojson",
  createCityStyleFn(baseStyles.Shemakha, (t) => t.replace(/ /g, "\n")),
  7.9999,
  10,
);
...
resultsContainer.style.cssText =
  "position:absolute;top:100%;left:0;width:100%;max-height:300px;overflow-y:auto;background:white;border:1px solid #895E8A;border-top:none;list-style:none;padding:0;margin:0;z-index:1000;display:none;";
...
async function loadCityData() {
  const promises = CITY_FILES.map(async (url) => {
    try {
      ...
      features.forEach((f) => {
        f.set("featureType", "title");
        searchSource.addFeature(f);
      });
    } catch (err) {
      console.error(`[Search] Ошибка загрузки ${url}:`, err);
    }
  });
...
searchInput.addEventListener("input", () => {
  ...
  const matches = searchSource
    .getFeatures()
    .filter((f) => safeGet(f, "title").toLowerCase().includes(term));
  matches.sort((a, b) => {
    const ta = safeGet(a, "title").toLowerCase(),
      tb = safeGet(b, "title").toLowerCase();
    if (ta === term) return -1;
    if (tb === term) return 1;
...
searchInput.addEventListener("keydown", (e) => {
  ...
  if (e.key === "ArrowDown") {
    ...
    (active === searchInput
      ...
      : active.previousElementSibling || searchInput
    ).focus();
  } else if (e.key === "Enter") {
    e.preventDefault();
    (active.tagName === "LI" ? active : items[0]).click();
  }
});
...
function handleInitialHash() {
  ...
  if (id) {
    const f = findFeatureById130(id);
    if (f) handleSearchSelect(f);
  }
}
...
function showCityDetails(feature) {
  if (!feature) return;
  const setTitle = (el, val) => {
    if (el) el.textContent = formatTitleForDisplay(val);
  };
  const setHTML = (el, val) => {
    if (el) el.innerHTML = val ? escapeHtml(String(val)) : "";
  };
  ...
  if (els.flag)
    els.flag.innerHTML = country
      ? `<img src="/icons/flags/${escapeHtml(country)}.png" title="${escapeHtml(country)}" width="17" height="17" style="display:inline-block;margin:0 5px;">`
      : "";

  if (els.googlelink)
    els.googlelink.innerHTML = `<a href="https://www.google.com/maps/place/${decLat},${decLon}" target="_blank" rel="noopener">Google Maps</a>`;
  if (els.OSMlink)
    els.OSMlink.innerHTML = `<a href="https://www.openstreetmap.org/index.html?mlat=${decLat}&mlon=${decLon}&zoom=16" target="_blank" rel="noopener">OpenStreetMap</a>`;
  if (els.wikimapialink)
    els.wikimapialink.innerHTML = `<a href="https://wikimapia.org/#lat=${decLat}&lon=${decLon}&z=16&l=0&m=w&v=0" target="_blank" rel="noopener">Wikimapia</a>`;

  const linkIf = (el, url, prefix) => {
    if (el)
      el.innerHTML = url
        ? `<a href="${escapeHtml(prefix)}${escapeHtml(url)}" target="_blank" rel="noopener">${escapeHtml(url)}</a>`
        : "";
  };
  linkIf(
    els.darelink,
    safeGet(feature, "dare"),
    "http://imperium.ahlfeldt.se/places/",
  );
  linkIf(
    els.pecslink,
    safeGet(feature, "PECS"),
    "http://www.perseus.tufts.edu/hopper/text?doc=Perseus:text:1999.04.0006:entry=",
  );
  linkIf(
    els.pleiadeslink,
    safeGet(feature, "pleiades"),
    "https://pleiades.stoa.org/places/",
  );
  linkIf(
    els.tpplacelink,
    safeGet(feature, "TTPlace"),
    "https://www.cambridge.org/us/talbert/talbertdatabase/TPPlace",
  );
  linkIf(
    els.topostextlink,
    safeGet(feature, "topostext"),
    "https://topostext.org/place/",
  );
  linkIf(els.vicilink, safeGet(feature, "vici"), "https://vici.org/vici/");
  linkIf(
    els.wikidatalink,
    safeGet(feature, "wikidata"),
    "https://www.wikidata.org/wiki/Q",
  );
  linkIf(
    els.wikipedialink,
    safeGet(feature, "wikipedia:en"),
  ...
  if (els.permLink) {
    els.permLink.href = permUrl;
    els.permLink.textContent = permUrl;
  }
  ...
  markersLayer = new VectorLayer({
    source: new VectorSource(),
    style: new Style({
      image: new Icon({ anchor: [0.5, 0.5], src: "/icons/target.png" }),
    }),
  });
  ...
  markersLayer
    .getSource()
    .addFeature(
      new Feature(new Point(feature.getGeometry().getFirstCoordinate())),
    );
...
map.on("click", (evt) => {
  const hitFeature = map.forEachFeatureAtPixel(
    evt.pixel,
    (f, l) => (cityLayers.includes(l) ? f : null),
    { hitTolerance: 3 },
  );
  if (hitFeature) {
    showCityDetails(hitFeature);
    if (els.workOpen) els.workOpen.click();
  } else {
...
window.addEventListener("load", () => {
  ...
  const update = () => {
    const pct = Math.min(
      100,
      Math.round(
        (Math.min(loaded, Math.max(total, 10)) / Math.max(total, 10)) * 100,
      ),
    );
  ...
  setTimeout(() => {
    ...
    if (text) text.textContent = "Starting map load...";
  }, 300);
});
